...
<template>
  <div class="list">
    <section
      v-for="(item, index) in list"
      :key="item.id"
      class="hot mx-auto mb-5 flex items-center rounded-lg bg-purple-300 px-3 py-3 shadow-md w-[900px]"
      @click="toggleHot(item.target.id)"
    >
      <span class="mr-3 flex items-center">
        <span class="align-middle">{{ index + 1 }}</span>
      </span>
      <div class="max-w-[600px] flex-grow pr-5">
      ...
      <img :src="item.children[0].thumbnail" alt="" class="ml-5 h-auto w-40 rounded-md">
    </section>
  </div>
</template>
